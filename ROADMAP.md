...
| Pilotos seguros | Runner automatizado, piloto manual guiado con checklist de captura, piloto de salida system con checklist de operador, piloto de transcripcion con checklist de revision y confirmacion humana de calidad, runbook, bundle doctor, analisis de bundles y checklist de beta |
...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, fallo estricto con `--fail-on-audit-gaps`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, checklist de revision para transcripcion real con `quality_review_confirmed`, checklist de operador para salida audible y motivos seguros para evidencias ignoradas.
