...
| Pilotos seguros | Runner automatizado, piloto manual guiado, piloto de salida system, runbook, bundle doctor, analisis de bundles y checklist de beta |
...
1. Generar fixture sintetico con `tools/pilot_audio_fixture.py --run-preflight`, validar ffmpeg, y ejecutar piloto de transcripcion real con audio propio no sensible usando `tools/transcription_pilot.py` con `--expected-text` o `--expected-text-file`, `--min-audio-seconds` y `--max-audio-seconds`.
