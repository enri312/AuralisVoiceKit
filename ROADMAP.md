...
| Pilotos seguros | Runner automatizado, piloto manual guiado con checklist de captura, piloto de salida system con checklist de operador y revision de voz, piloto de transcripcion con checklist de revision y confirmacion humana de calidad, runbook, bundle doctor, analisis de bundles y checklist de beta |
...
- Runner de piloto manual guiado. Estado: inicial con `tools/manual_pilot.py`, `manual-capture-checklist.md`, `capture_checklist`, `system_guard`, `--expected-system`, redaccion de selector de dispositivo y sin captura real salvo `--capture-test`.
...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, fallo estricto con `--fail-on-audit-gaps`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, checklist de revision para transcripcion real con `quality_review_confirmed`, checklist de operador para salida audible con `voice_review_confirmed` y motivos seguros para evidencias ignoradas.
...
2. Preparar `output-operator-checklist.md` con `tools/output_pilot.py --output-dir pilot_runs/output/system-dry-run --json` y ejecutar piloto manual de salida `system` con `tools/output_pilot.py --speak --operator-present --confirm-audible --confirm-voice-reviewed --output-dir pilot_runs/output/system-real` solo despues de revisar voz, volumen y pronunciacion.
3. Repetir captura con microfono en Ubuntu/Linux y macOS usando `--expected-system` y conservando `manual-capture-checklist.md`.
