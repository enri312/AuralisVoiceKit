...
Nota `v0.144.0`: `tools/pilot_run.py` incorpora `real-pilot-rehearsal-card.md` y `real_pilot_rehearsal_card` para ensayar el siguiente piloto real sin hardware, microfono, audio real, modelos ni evidencia beta antes de copiar comandos locales.

Nota `v0.145.0`: `tools/pilot_run.py` incorpora `real-pilot-evidence-package.md` y `real_pilot_evidence_package_card` para reunir artifacts sanitizados, campos requeridos/faltantes, directorios sugeridos y comandos de auditoria/refresco despues del piloto real.

Nota `v0.146.0`: `tools/pilot_run.py` incorpora `real-pilot-operator-brief.md` y `real_pilot_operator_brief_card` como resumen publico de una pagina con foco activo, comando plantilla, lecturas previas, confirmaciones humanas, pendientes de copia local, alto operativo y auditoria posterior.
...
Nota `v0.170.0`: `tools/release_batch_status.py` agrega `batch_state`, `batch_summary_*` y `publishable_commits_needed` para distinguir lotes recien reiniciados, en progreso o listos antes de crear tags.

Nota `v0.170.0`: `tools/release_batch_status.py` agrega `publish_decision`, `release_blocker` y `explicit_user_override_required` para que automatizaciones y operadores mantengan el release en espera mientras el lote este incompleto.

Nota `v0.170.0`: `tools/pilot_run.py` propaga ese estado explicito del lote a `pilot-plan.md`, `real-pilot-handoff.md` y `real-pilot-decision-gate.md`, mostrando resumen y commits publicables faltantes para el operador.

Nota `v0.170.0`: las tarjetas finales de operador (`real-pilot-operator-brief.md`, `real-pilot-run-sheet.md`, `real-pilot-final-go-no-go.md` y `real-pilot-local-receipt.md`) incluyen `release_batch_notice` con `publish_decision`, `release_blocker` y `explicit_user_override_required` para separar resultados de pilotos reales de la decision de publicar tags.
