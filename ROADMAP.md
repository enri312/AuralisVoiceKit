...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit` y motivos seguros para evidencias ignoradas.
...
- Pilotos seguros. Estado: runner automatizado, piloto manual guiado con `tools/manual_pilot.py`, piloto de salida `system` con `tools/output_pilot.py`, piloto de transcripcion con `tools/transcription_pilot.py`, scoring redactado con `--expected-text`, checklist de beta con `tools/beta_readiness.py`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, motivos seguros para evidencias ignoradas, runbook inicial con `PILOTS.md`, hallazgos en `PILOT_FINDINGS.md`, blockers en `BETA_CHECKLIST.md`, bundles `doctor` sanitizados, analisis `doctor-bundles`, control explicito de sample rate para pilotos WASAPI, primera captura Windows real aprobada a 48000 Hz, dry-run Windows de salida `system` aprobado, guard `--operator-present` listo para audio real y dry-run Windows de transcripcion sintetica aprobado.
