...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, fallo estricto con `--fail-on-audit-gaps`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, checklist de captura real con `input_review_confirmed`, checklist de revision para transcripcion real con `audio_review_confirmed`, `reference_review_confirmed`, `reference_privacy_scan.passed` y `quality_review_confirmed`, checklist de operador para salida audible con `text_review_confirmed`, `spoken_text_privacy_scan.passed` y `voice_review_confirmed`, guard `system_guard.expected_system_matched` para salida audible y motivos seguros para evidencias ignoradas.
...
1. Generar fixture sintetico con `tools/pilot_audio_fixture.py --run-preflight`, validar ffmpeg, revisar `transcription-review-checklist.md`, y ejecutar piloto de transcripcion real con audio propio no sensible usando `tools/transcription_pilot.py` con `--expected-text` o `--expected-text-file`, `--min-audio-seconds`, `--max-audio-seconds`, `--confirm-audio-reviewed`, `--confirm-reference-reviewed`, `reference_privacy_scan.passed=true` y `--confirm-quality-reviewed` solo despues de revisar privacidad del audio, privacidad de la referencia y calidad localmente.
