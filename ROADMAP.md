...
1. Ejecutar piloto de transcripcion real con audio propio no sensible usando `tools/transcription_pilot.py` con `--expected-text` o `--expected-text-file`, `--min-audio-seconds` y `--max-audio-seconds`.
