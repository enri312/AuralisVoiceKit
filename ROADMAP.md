...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, fallo estricto con `--fail-on-audit-gaps`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, checklist de captura real con `input_review_confirmed`, checklist de revision para transcripcion real con `audio_review_confirmed`, `reference_review_confirmed` y `quality_review_confirmed`, checklist de operador para salida audible con `voice_review_confirmed`, guard `system_guard.expected_system_matched` para salida audible y motivos seguros para evidencias ignoradas.
