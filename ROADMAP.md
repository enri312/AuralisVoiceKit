...
2. Preparar `output-operator-checklist.md` con `tools/output_pilot.py --output-dir pilot_runs/output/system-dry-run --json` y ejecutar piloto manual de salida `system` con `tools/output_pilot.py --speak --operator-present --confirm-audible --confirm-text-reviewed --confirm-voice-reviewed --expected-system "Windows|Linux|Darwin" --output-dir pilot_runs/output/system-real` solo despues de revisar privacidad del texto, `spoken_text_privacy_scan.passed=true`, voz, volumen y pronunciacion.
