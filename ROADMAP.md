...
| Pilotos seguros | Runner automatizado, piloto manual guiado con checklist de captura y revision confirmada de entrada, piloto de salida system con checklist de operador, guard de plataforma y revision de voz, piloto de transcripcion con checklist de revision y confirmacion humana de calidad, runbook, bundle doctor, analisis de bundles y checklist de beta |
...
- Checklist de bugs conocidos. Estado: inicial con `tools/beta_readiness.py`, `BETA_CHECKLIST.md`, ingesta de evidencias JSON con `--evidence`, requisitos de evidencias con `--requirements`, auditoria de artifacts con `--audit-evidence`, fallo estricto con `--fail-on-audit-gaps`, resumen de blockers cerrados/pendientes por evidencia, validacion de `project: AuralisVoiceKit`, checklist de captura real con `input_review_confirmed`, checklist de revision para transcripcion real con `audio_review_confirmed`, `reference_review_confirmed`, `reference_privacy_scan.passed` y `quality_review_confirmed`, checklist de operador para salida audible con `voice_review_confirmed`, guard `system_guard.expected_system_matched` para salida audible y motivos seguros para evidencias ignoradas.
...
2. Preparar `output-operator-checklist.md` con `tools/output_pilot.py --output-dir pilot_runs/output/system-dry-run --json` y ejecutar piloto manual de salida `system` con `tools/output_pilot.py --speak --operator-present --confirm-audible --confirm-voice-reviewed --expected-system "Windows|Linux|Darwin" --output-dir pilot_runs/output/system-real` solo despues de revisar voz, volumen y pronunciacion.
