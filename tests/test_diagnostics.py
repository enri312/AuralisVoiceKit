...
class DiagnosticsTests(unittest.TestCase):
    ...
    def test_run_doctor_can_override_capture_sample_rate(self):
        report = run_doctor(
            include_capture_test=True,
            capture_backend="null",
            capture_test_seconds=0.001,
        ...
        check = checks["capture-test:null"]
        self.assertEqual(check.status, DiagnosticStatus.OK)
        self.assertEqual(check.details["sample_rate"], 48000)
        self.assertEqual(check.details["capture_block_frames"], 2400)
    ...
    def test_run_doctor_rejects_invalid_capture_sample_rate(self):
        report = run_doctor(
            include_capture_test=True,
            capture_backend="null",
            capture_test_seconds=0.001,
        ...
        self.assertEqual(report.status, DiagnosticStatus.ERROR)
        self.assertEqual(check.status, DiagnosticStatus.ERROR)
        self.assertEqual(check.details["requested_sample_rate"], 0)
