...
class OutputPilotTests(unittest.TestCase):
    def test_output_pilot_writes_sanitized_report_and_findings(self):
        ...
        self.assertEqual(report["target_output_backend"]["freedom_policy"]["category"], "system-local")
        self.assertFalse(report["target_output_backend"]["freedom_policy"]["network_required"])
        self.assertIn("Target output backend freedom policy: system-local", findings)
        self.assertIn("Target output backend freedom policy: system-local", next_step)
        ...
        self.assertFalse(report["target_output_backend"]["readiness_plan"]["uses_pip_extra"])
        self.assertIsNone(report["target_output_backend"]["readiness_plan"]["python_extra"])
        self.assertIsNone(report["target_output_backend"]["readiness_plan"]["pip_command"])
        ...
        self.assertIn("Target output backend readiness uses pip extra: False", findings)
        ...
        self.assertIn("Target output backend readiness uses pip extra: False", next_step)
        ...
        self.assertFalse(report["system_output_command_card"]["uses_pip_extra"])
        self.assertIsNone(report["system_output_command_card"]["python_extra"])
        self.assertIsNone(report["system_output_command_card"]["pip_command"])
        ...
        self.assertTrue(report["system_output_command_card"]["system_dependency_plan"]["safe_to_share"])
        self.assertFalse(report["system_output_command_card"]["system_dependency_plan"]["post_install_check_plays_audio"])
        self.assertFalse(report["system_output_command_card"]["system_dependency_plan"]["records_local_paths"])
        self.assertEqual(report["system_output_command_card"]["system_dependency_plan"]["system"], "Darwin")
        self.assertFalse(report["system_output_command_card"]["system_dependency_plan"]["uses_pip_extra"])
        self.assertIsNone(report["system_output_command_card"]["system_dependency_plan"]["python_extra"])
        self.assertIsNone(report["system_output_command_card"]["system_dependency_plan"]["pip_command"])
        ...
        self.assertEqual(
            report["system_output_operator_gate"]["copy_readiness"]["decision"],
            "ready_to_copy_template",
        )
        self.assertTrue(report["system_output_operator_gate"]["copy_readiness"]["template_safe_to_copy"])
        self.assertFalse(report["system_output_operator_gate"]["copy_readiness"]["ready_to_run_real_audio"])
        self.assertFalse(report["system_output_operator_gate"]["copy_readiness"]["records_spoken_text"])
        self.assertFalse(report["system_output_operator_gate"]["copy_readiness"]["records_operator_identity"])
        self.assertIn(
            "<public-spoken-text>",
            report["system_output_operator_gate"]["copy_readiness"]["required_placeholders"],
        )
        self.assertIn(
            "real_output_explicitly_requested",
            report["system_output_operator_gate"]["copy_readiness"]["pending_confirmations"],
        )
        self.assertIn(
            "real_audio_requested",
            report["system_output_operator_gate"]["copy_readiness"]["pending_fields"],
        )
        ...
        self.assertIn("Command card uses pip extra: False", next_step)
        self.assertIn("Command card system dependency plan safe to share: True", next_step)
        self.assertIn("Command card system dependency post-install plays audio: False", next_step)
        self.assertIn("System dependency plan:", next_step)
        self.assertIn("Uses pip extra: False", next_step)
        self.assertIn("Command card records spoken text: False", next_step)
        self.assertIn("System output command card uses pip extra: False", findings)
        self.assertIn("System output dependency plan safe to share: True", findings)
        ...
        self.assertIn("System output copy readiness decision: ready_to_copy_template", findings)
        self.assertIn("Template safe to copy: True", checklist)
        self.assertIn("Ready to run real audio: False", next_step)
    ...
    def test_output_pilot_real_audio_gap_can_be_ready_with_sanitized_fake_backend(self):
        ...
        self.assertTrue(report["system_output_operator_gate"]["copy_readiness"]["template_safe_to_copy"])
        self.assertTrue(report["system_output_operator_gate"]["copy_readiness"]["ready_to_run_real_audio"])
        self.assertEqual(report["system_output_operator_gate"]["copy_readiness"]["pending_confirmations"], [])
        self.assertEqual(report["system_output_operator_gate"]["copy_readiness"]["pending_fields"], [])
        self.assertFalse(report["system_output_operator_gate"]["records_spoken_text"])
        self.assertFalse(report["system_output_operator_gate"]["records_operator_identity"])
        self.assertFalse(report["beta_evidence_gap"]["records_spoken_text"])
        self.assertFalse(report["beta_evidence_gap"]["records_operator_identity"])
        ...
        self.assertIn("Ready to run real audio: True", next_step)
    ...
    def test_output_pilot_cli_outputs_json(self):
        ...
        self.assertFalse(payload["target_output_backend"]["readiness_plan"]["uses_pip_extra"])
        self.assertIsNone(payload["target_output_backend"]["readiness_plan"]["python_extra"])
        self.assertIsNone(payload["target_output_backend"]["readiness_plan"]["pip_command"])
    ...
    def test_output_backend_readiness_plan_supports_target_platforms(self):
        ...
        self.assertFalse(windows["uses_pip_extra"])
        self.assertIsNone(windows["python_extra"])
        self.assertIsNone(windows["pip_command"])
        self.assertIn("System.Speech", " ".join(windows["platform_notes"]))
        self.assertTrue(linux["requires_package_manager"])
        self.assertFalse(linux["uses_pip_extra"])
        self.assertIsNone(linux["python_extra"])
        self.assertIsNone(linux["pip_command"])
