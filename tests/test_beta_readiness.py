...
class BetaReadinessTests(unittest.TestCase):
    ...
    def test_evidence_audit_explains_satisfied_and_missing_fields(self):
        ...
        self.assertEqual(report["privacy_remediation_plan"]["status"], "not_required")
        self.assertEqual(report["privacy_remediation_plan"]["step_count"], 0)
        self.assertFalse(report["privacy_remediation_plan"]["records_private_values"])
    ...
    def test_cli_audit_evidence_markdown_is_public_safe(self):
        ...
        self.assertIn("Plan de remediacion de privacidad", content)
    ...
    def test_cli_audit_evidence_json_ignores_beta_blocker_failure(self):
        ...
        self.assertEqual(payload["privacy_remediation_plan"]["status"], "not_required")
    ...
    def test_evidence_audit_blocks_raw_private_fields_without_printing_values(self):
        ...
        self.assertEqual(report["privacy_remediation_plan"]["status"], "required")
        self.assertEqual(report["privacy_remediation_plan"]["step_count"], 2)
        self.assertTrue(report["privacy_remediation_plan"]["blocking"])
        self.assertTrue(report["privacy_remediation_plan"]["safe_to_share"])
        self.assertFalse(report["privacy_remediation_plan"]["records_private_values"])
        ...
        steps = {step["field"]: step for step in report["privacy_remediation_plan"]["steps"]}
        self.assertEqual(steps["transcript.text"]["safe_replacement"], "<text-redacted>")
        self.assertEqual(steps["audio.path"]["safe_replacement"], "<path-redacted>")
    ...
    def test_cli_audit_evidence_can_fail_on_privacy_findings(self):
        ...
        self.assertEqual(payload["privacy_remediation_plan"]["status"], "required")
        self.assertEqual(payload["privacy_remediation_plan"]["steps"][0]["field"], "expected_text")
