...
class TranscriptionPilotTests(unittest.TestCase):
    ...
    def test_transcription_pilot_preflight_decodes_audio_without_backend(self):
        ...
        self.assertTrue(report["target_backend"]["install_plan"]["uses_pip_extra"])
        self.assertEqual(report["target_backend"]["install_plan"]["python_extra"], "whisper")
        ...
        self.assertEqual(report["target_backend"]["freedom_policy"]["category"], "free-local")
        self.assertFalse(report["target_backend"]["freedom_policy"]["proprietary"])
        self.assertFalse(report["target_backend"]["freedom_policy"]["network_required"])
        self.assertIn("Target backend freedom policy: free-local", findings)
        self.assertIn("Target backend freedom policy: free-local", next_step)
        ...
        self.assertIn("real_transcription_requested", report["beta_evidence_gap"]["missing_fields"])
        self.assertIn("transcript.text_redacted", report["beta_evidence_gap"]["missing_fields"])
    ...
    def test_transcription_pilot_openai_preflight_requires_sanitized_api_key(self):
        ...
        self.assertFalse(report["beta_evidence_gap"]["ready_for_beta_evidence"])
        self.assertIn("credentials.openai_api_key_present", report["beta_evidence_gap"]["missing_fields"])
    ...
    def test_transcription_pilot_preflight_decision_blocks_without_duration_gate(self):
        ...
        self.assertIn("local Whisper path", report["preflight_decision"]["next_action"])
        self.assertIn("optional proprietary API integration", report["preflight_decision"]["next_action"])
        self.assertNotIn("Whisper or OpenAI", report["preflight_decision"]["next_action"])
    ...
    def test_transcription_pilot_cli_preflight_allows_target_backend(self):
        ...
        self.assertTrue(payload["target_backend"]["install_plan"]["uses_pip_extra"])
        self.assertEqual(payload["target_backend"]["install_plan"]["python_extra"], "whisper")
    ...
    def test_transcription_backend_install_plan_supports_real_backends(self):
        ...
        self.assertTrue(whisper["uses_pip_extra"])
        self.assertEqual(whisper["python_extra"], "whisper")
        self.assertTrue(openai["uses_pip_extra"])
        self.assertEqual(openai["python_extra"], "openai")
        self.assertFalse(null_backend["uses_pip_extra"])
        self.assertIsNone(null_backend["python_extra"])
