...
`tools/beta_readiness.py` resume blockers de beta a partir del gate, `PILOT_FINDINGS.md` y artifacts JSON pasados con `--evidence`. Hoy marca como pendientes la transcripcion real con calidad y `transcription_checklist.ready_for_beta_evidence=true`, salida `system` audible confirmada con `operator_checklist.ready_for_beta_evidence=true`, captura Ubuntu/Linux y captura macOS con `capture_checklist.ready_for_beta_evidence=true`.
...
3. Repetir captura con microfono en Ubuntu/Linux y macOS y conservar `manual-capture-checklist.md`.
