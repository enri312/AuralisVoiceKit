...
`tools/beta_readiness.py` resume blockers de beta a partir del gate, `PILOT_FINDINGS.md` y artifacts JSON pasados con `--evidence`. Hoy marca como pendientes la transcripcion real con calidad y `transcription_checklist.ready_for_beta_evidence=true`, salida `system` audible confirmada con `operator_checklist.ready_for_beta_evidence=true`, captura Ubuntu/Linux y captura macOS con `system_guard.expected_system_matched=true` y `capture_checklist.ready_for_beta_evidence=true`.
...
1. Ejecutar piloto de transcripcion real con audio propio no sensible usando `tools\transcription_pilot.py --real-transcription --audio ... --audio-non-sensitive --expected-text ... --min-word-accuracy 0.75 --min-audio-seconds 0.2 --max-audio-seconds 60` y revisar `transcription-review-checklist.md`.
