...
Si el conteo es menor a 5, se puede hacer commit y push, pero se deja la version como pendiente/desarrollo y no se dispara release. El JSON incluye `batch_state` (`fresh`, `collecting` o `ready`), `publishable_commits_needed`, `publish_decision`, `release_blocker` y `explicit_user_override_required` para automatizar la decision sin crear tags prematuros. English: alpha releases are batched; tag only after 5 publishable improvements unless explicitly requested.
