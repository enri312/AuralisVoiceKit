...
Nota `v0.158.0`: `target_output_backend.readiness_plan` en pilotos de salida `system` declara `uses_pip_extra=false`, `python_extra=null` y `pip_command=null`, de modo que el readiness, la command card y la auditoria beta comparten el mismo contrato. English: system output readiness plans now carry structured no-pip-extra evidence.

Nota `v0.159.0`: `BETA_CHECKLIST.md` ahora muestra como evidencia faltante los tres campos del readiness plan de salida `system`: `Target output backend readiness uses pip extra: False`, `Target output backend readiness python extra: not-set` y `Target output backend readiness pip command: not-set`. English: beta checklists now surface missing system-output readiness evidence.
...
Nota `v0.162.0`: `real_pilot_command_pack` y `real-pilot-command-pack.md` tambien exponen `system_output_no_pip_extra_contract` antes de los comandos copiables. English: the real-pilot command pack now carries the system-output no-pip-extra contract before copyable commands.
...
Nota `v0.131.0`: el piloto seguro escribe `real-pilot-hard-stop-card.md` con condiciones de alto, acciones minimas y politica publica antes de usar hardware, audio real, texto hablado real o flags `--confirm-*`. English: safe pilots now include a hard-stop card for real-world operators.

Nota `v0.132.0`: el piloto seguro escribe `real-pilot-evidence-intake-card.md` con directorios sugeridos, artifacts JSON aceptados, comandos de auditoria/refresco y reglas para ingerir solo evidencia sanitizada. English: safe pilots now include a public-safe evidence intake card.
...
Nota `v0.138.0`: `tools/beta_readiness.py` exige `capture_operator_gate.ready_for_beta_audit=true`, sin confirmaciones ni campos faltantes, para aceptar evidencia beta de captura. English: beta capture evidence now requires the manual capture operator gate.
...
Nota `v0.142.0`: `operator_gate.command_audit.copy_safety` separa plantilla segura de copia, bloqueos tecnicos y revisiones locales pendientes antes de ejecutar el comando del siguiente piloto real. English: operator gates now expose copy-safety status before real-pilot command execution.

Nota `v0.143.0`: el piloto seguro escribe `real-pilot-audit-closure.md` y `real_pilot_audit_closure_card` para cerrar cada piloto real con auditoria estricta, refresco controlado de `BETA_CHECKLIST.md` y hallazgos sanitizados. English: safe pilots now include a public-safe audit closure card after real evidence is generated.

Nota `v0.144.0`: el piloto seguro escribe `real-pilot-rehearsal-card.md` y `real_pilot_rehearsal_card` para ensayar localmente con comandos seguros antes de copiar o ejecutar el comando real. English: safe pilots now include a public-safe rehearsal card before real hardware/audio work.

Nota `v0.145.0`: el piloto seguro escribe `real-pilot-evidence-package.md` y `real_pilot_evidence_package_card` para reunir JSON real sanitizado, hallazgos, checklist beta y auditoria estricta antes de contar avances beta. English: safe pilots now include a public-safe sanitized evidence package card.

Nota `v0.146.0`: el piloto seguro escribe `real-pilot-operator-brief.md` y `real_pilot_operator_brief_card` como resumen seguro de una pagina para el operador local antes de ejecutar el siguiente piloto real. English: safe pilots now include a public-safe local operator brief.

Nota `v0.147.0`: el piloto seguro escribe `real-pilot-run-sheet.md` y `real_pilot_run_sheet_card` como hoja segura por fases para ejecutar el siguiente piloto real con prerequisitos, ensayo, consentimiento/copia, evidencia y auditoria. English: safe pilots now include a public-safe local run sheet.
...
- Pilotos manuales con salida `system` real: runner preparado con `tools/output_pilot.py`; dry-run Windows aprobado, tarjeta `real-pilot-system-output-readiness.md`, `output-operator-checklist.md`, `system-output-next-step.md`, `system_guard.expected_system_matched`, `target_output_backend.available`, `target_output_backend.readiness_plan`, `output_backend_ready_required`, `text_review_confirmed`, `spoken_text_privacy_scan.passed`, `operator_checklist.expected_system_matched`, `operator_checklist.records_operator_identity=false`, `operator_checklist.redacts_spoken_text=true`, `operator_checklist.text_review_confirmed`, `operator_checklist.spoken_text_privacy_scan_passed`, `operator_checklist.voice_review_confirmed`, `operator_checklist.commands_available=true`, `operator_checklist.ready_for_real_audio=true`, `operator_checklist.ready_for_beta_evidence`, `next_system_output.uses_placeholders=true`, `next_system_output.records_spoken_text=false`, `system_output_operator_gate.ready_for_beta_audit`, `system_output_operator_gate.command_safe_to_copy`, `system_output_operator_gate.missing_confirmation_count=0` y `system_output_operator_gate.records_spoken_text=false` listos, guards `--operator-present`, `--confirm-audible`, `--confirm-text-reviewed`, `--confirm-voice-reviewed`, `--require-output-backend-ready` y `--expected-system` listos, audio real pendiente con operador presente.
- Pilotos manuales con transcripcion real: runner preparado con `tools/transcription_pilot.py`; fixture sintetico publico con `preflight.freedom_policy`, tarjeta `real-pilot-fixture-preflight.md` con comandos, artifacts esperados, estado de ffmpeg y condiciones de alto, tarjeta `real-pilot-transcription-readiness.md` con backend objetivo, `target_backend.install_plan`, guard `--require-target-backend-ready`, revisiones de audio/referencia/calidad y condiciones de alto, dry-run sintetico Windows, preflight con `target_backend.available`, `target_backend_ready_required`, dependencias, comando de instalacion del extra opcional, `preflight_decision`, `--require-target-backend-ready`, `transcription-review-checklist.md`, `real-transcription-next-step.md`, scoring redactado, scan redactado `reference_privacy_scan.passed`, requisito beta `target_backend.available=true`, `target_backend_ready_required=true`, `audio.generated_synthetic_audio=false`, `audio.audio_confirmed_non_sensitive=true`, `audio.decoded=true`, redaccion `audio.audio_file_name_redacted`, `audio.duration_gate.enabled=true`, `audio.duration_gate.passed=true`, `transcript.text_redacted=true`, `transcription_checklist.records_audio_file_name=false`, `transcription_checklist.records_expected_text_file_name=false`, `transcription_checklist.redacts_transcript_text=true`, `transcription_checklist.redacts_expected_text=true`, guardas de duracion, `--confirm-audio-reviewed`, `--confirm-reference-reviewed` y `--confirm-quality-reviewed` preparados, audio real pendiente con archivo no sensible. Whisper queda visible como `free-local`; cualquier API propietaria queda marcada como opt-in con red requerida.
